...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	WHERE id = $5
	`
	result, err := tx.Exec(updateQuery, workout.Title, workout.Description, workout.DurationMinutes, workout.CaloriesBurned, workout.ID)
	if err != nil {
		return err
	}
